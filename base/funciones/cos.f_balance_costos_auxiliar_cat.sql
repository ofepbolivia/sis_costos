...
CREATE OR REPLACE FUNCTION cos.f_balance_costos_auxiliar_cat (
  p_desde date,
  p_hasta date,
...
BEGIN
    ...
    v_nombre_funcion = 'cos.f_balance_costos_auxiliar_cat';
    ...
    if p_incluir_cierre = 'todos' then
     ...
     insert  into temp_balance_costos (
                        ...
                        inner join pre.tpresupuesto p on p.id_presupuesto = t.id_centro_costo
                        inner join pre.tcategoria_programatica cat on cat.id_categoria_programatica = p.id_categoria_prog
                        where 
